...
class CrashPrediction(Base):
    ...
    crash_probability = Column(Float)  # 0-1
    horizon_days = Column(Integer)  # Usually 20
    target_date = Column(DateTime)
    actual_crash = Column(Boolean, nullable=True)  # Filled later
...
class StockPrediction(Base):
    ...
    our_confidence = Column(Float)  # 0-1
    created_at = Column(DateTime, default=datetime.utcnow)
...
class AccuracyLog(Base):
    ...
    model_type = Column(String)  # crash/price/etc
...
class MarketRegime(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    date = Column(DateTime, index=True)
    regime = Column(String)  # bull/bear/volatile/crisis
    vix = Column(Float)
...
class SectorRotation(Base):
    ...
    sectors_to_buy = Column(Text)  # JSON array
    sectors_to_sell = Column(Text)  # JSON array
